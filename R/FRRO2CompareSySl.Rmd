...
O2SlopeDataClean <- O2SlopeDataClean %>%
  mutate(ActPARCorrinter = interpolate(ActPARCorr))
         ...
         #LR_sFRRfinter = interpolate(LR_s), .after = Temp_C)
...
O2Slopes <- O2SlopeDataClean %>%
    nest(O2data = -c(Date, Run, ActPARCorrinter, FilenameO2, CultureID, ObsDate, Ex_WL, GrowthLight, TempCont)) %>%
...
#%>%
  #select(-c("culture_inocul_L", "media_inocul_L", "target_media_id", "inoc_media_id", "nutrient_status", "imipenem_ug_mL", "cefoxitin_ug_mL", "meropenem_ug_mL", "ampicillin_ug_mL"))
...
O2FRRfMeta %>%
  filter(ETRCtauav > 0) %>%
  ggplot() +
  geom_point(aes(x = O2evln_O2lsMIN, y = ETRCtauav, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Strain)) +
  theme_bw()
...
O2FRRfMeta %>%
  filter(ETRqpOxbo > 0) %>%
  ggplot() +
  geom_point(aes(x = O2evln_O2lsMIN, y = ETRqpOxbo, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
...
O2FRRfMeta %>%
  filter(ETRqpOxbo > 0) %>%
  ggplot() +
  geom_point(aes(y  = (O2evln_O2lsMIN * 4)/ETRqpOxbo, x = ActPARCorr, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Strain)) +
...
O2FRRfMeta %>%
  filter(ETRCtauav > 0) %>%
  ggplot() +
  geom_point(aes(y  = (O2evln_O2lsMIN * 4)/ETRCtauav, x = ActPARCorr, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Strain)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +
  theme_bw()
...
O2FRRfMeta %>%
  filter(ETRGorbo > 0) %>%
  ggplot() +
  geom_point(aes(y  = (O2evln_O2lsMIN * 4)/ETRGorbo, x = ActPARCorr, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Strain)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +
  theme_bw()
...
XXXXProgress using minimum O2 rate rather than rate at 0 lightXXXXX
